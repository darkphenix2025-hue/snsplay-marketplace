---
name: commit-push-pr
description: Commit changes, push to GitHub, open a PR, and auto-merge to main. Includes quality checks (security, patterns, simplification). Use --quick to skip checks, --no-merge to skip auto-merge.
...
**Note about merge flags:**
- `--no-merge`: Skip the auto-merge step (Step 9). PR will remain open for review.
- Default behavior (no flag): Auto-merge PR to main after creation.
...
### Step 9: Auto-Merge PR to Main (New Feature Branch Only)

**Applies only when:**
- A new PR was just created (not an existing PR)
- The PR was created from a feature branch (not on default branch)
- No `--no-merge` flag was passed

**Merge Strategy:** Use squash merge to keep main history clean.

1. Wait a moment for PR to be ready:
```bash
sleep 2
```

2. Check PR mergeability status:
```bash
gh pr view "$(gh pr list --head "$(git branch --show-current)" --json number --jq '.[0].number')" --json mergeable,number
```

3. If mergeable, perform squash merge with automatic branch deletion:
```bash
...
5. Report the merge:
```
PR #<number> merged to $DEFAULT_BRANCH via squash merge
Feature branch deleted
Local $DEFAULT_BRANCH updated
```

**If merge fails:**
- `CONFLICT`: Report conflict, suggest manual resolution
- `NOT_MERGEABLE`: Report the reason (failing checks, etc.)
- `PERMISSION`: Suggest user merge manually on GitHub

**Skip merge when:**
- `--no-merge` flag is passed
- PR already has `needs-work` or `draft` status
- User explicitly requests to keep PR open for review
...
Merged: PR #123 merged to main (squash)
Branch deleted: feat-cli-add-project-list-command
Local main updated
```

On success without merge (--no-merge):
```
Committed: feat(cli): add project list command
Branch: feat-cli-add-project-list-command
Pushed to: origin/feat-cli-add-project-list-command
PR: https://github.com/owner/repo/pull/123
PR created (not merged per --no-merge flag)
```
